...
def tick(ticker):
    cookies = getcookie()
    url = f'https://www.nseindia.com/search?q={ticker}&type=quotes'

    response = requests.get(url, headers=headers, cookies=cookies)
    soup = BeautifulSoup(response.text, 'html.parser')
    resultsDiv = soup.find_all('div', {'class': 'searchLink'})
    ...
    if company_ticker == 'RELIANCEP1':
        company_ticker = 'RELIANCE'

    return company_ticker

@app.route('/ticker/<inp>')
def companyData(inp):
    ticker_name = tick(inp)
    cookies = getcookie()
    url_api = f'https://www.nseindia.com/api/option-chain-equities?symbol={ticker_name}'
    response = requests.get(url_api, headers=headers, cookies=cookies)
    data = response.json()

    if len(data) != 0:
        filtered = data['filtered']
        ce_totOI = filtered['CE']['totOI']
        pe_totOI = filtered['PE']['totOI']
        ce_totCOI = []
        pe_totCOI = []

        for i in filtered['data']:
            if 'CE' in i:
                ce_totCOI.append(i['CE']['changeinOpenInterest'])
            if 'PE' in i:
                pe_totCOI.append(i['PE']['changeinOpenInterest'])

        ce_totCOI = sum(ce_totCOI)
        pe_totCOI = sum(pe_totCOI)
        ...
        result_data = jsonify(result)
        result_data.headers.add('Access-Control-Allow-Origin','*')
        return result_data
    else:
        result_data = jsonify(f"Option Chain Equity data not available for {ticker_name}")
        result_data.headers.add('Access-Control-Allow-Origin','*')
        return result_data
...
def data2():
    ...
    nf_price = int(round(float(nf_price)/1000, 1)*1000)
    ...
    for i in bnf_data['data']:
        if (bnf_price-1000) <= i['strikePrice'] <= (bnf_price+1000):
            bnf_strike.append(i['strikePrice'])
            if 'CE' in i:
                bnf_ce_coi.append(i['CE']['changeinOpenInterest'])
                bnf_ce_vol.append(i['CE']['totalTradedVolume'])
    ...
    for j in nf_data['data']:
        if (nf_price-1000) <= j['strikePrice'] <= (nf_price+1000):
            nf_strike.append(j['strikePrice'])
            if 'CE' in j:
                nf_ce_coi.append(j['CE']['changeinOpenInterest'])
                nf_ce_vol.append(j['CE']['totalTradedVolume'])
